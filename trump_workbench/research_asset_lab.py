...
from .config import DEFAULT_ETF_SYMBOLS, AppSettings
...
from .research_workspace import _figure_json, _frame_records, _public_filter_payload, _resolve_filters, detect_source_mode
...
def _records(frame: pd.DataFrame, limit: int | None = TABLE_LIMIT) -> list[dict[str, Any]]:
    if frame.empty:
        return []
    return _frame_records(frame.head(limit) if limit is not None else frame)


def _comparison_mode(value: str | None) -> str:
...
def _asset_options(asset_universe: pd.DataFrame, asset_daily: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for symbol in sorted(symbol for symbol in daily_symbols if symbol != "SPY" and symbol not in seen):
        options.append(
            {
                "symbol": symbol,
                "label": symbol,
...
def _select_asset(options: list[dict[str, Any]], selected_asset: str | None) -> str:
    symbols = [str(option["symbol"]).upper() for option in options]
...
def _benchmark_options(selected_asset: str) -> list[str]:
    selected = str(selected_asset or "").upper()
...
def _select_benchmark(selected_asset: str, benchmark_symbol: str | None) -> str:
    ...
    if requested in {"", "NONE", "NULL"}:
        return "None"
    return requested if requested in _benchmark_options(selected_asset) else "None"


def _normalize_daily_market(asset_daily: pd.DataFrame, date_start: pd.Timestamp, date_end: pd.Timestamp) -> pd.DataFrame:
    if asset_daily.empty or not {"symbol", "trade_date"}.issubset(asset_daily.columns):
...
def _filter_asset_mappings(
    ...
    if asset_post_mappings.empty or not {"asset_symbol", "session_date"}.issubset(asset_post_mappings.columns):
...
def _filter_asset_features(
    ...
    if asset_session_features.empty or not {"asset_symbol", "signal_session_date"}.issubset(asset_session_features.columns):
...
def _normalize_intraday_frame(intraday: pd.DataFrame, timezone: str) -> pd.DataFrame:
    if intraday.empty or not {"symbol", "timestamp"}.issubset(intraday.columns):
...
def _intraday_anchor_options(
    ...
    intraday_dates = {
        symbol: set(intraday.loc[intraday["symbol"] == symbol, "timestamp"].dropna().dt.date.tolist())
    ...
    if not eligible_sessions:
        return [], mappings.head(0).copy(), (
            f"No recent intraday coverage overlaps mapped sessions for {', '.join(sorted(required_symbols))}."
        )

    eligible = mappings.loc[pd.to_datetime(mappings["session_date"], errors="coerce").dt.date.isin(eligible_sessions)].copy()
    ...
    message = "" if options else "Mapped posts exist, but none have a usable intraday anchor timestamp."
    return options, eligible, message
...
def _empty_payload(
    *,
    ...
    return {
        ...
        "filters": _public_filter_payload(filters),
        "controls": {
        ...
        "benchmark_options": _benchmark_options(selected_asset),
        "headline_metrics": {},
        "charts": {
            "overlay": _figure_json(build_asset_comparison_chart(pd.DataFrame(), selected_asset or "asset", comparison_mode)),
            "event_study": _figure_json(build_event_study_chart(pd.DataFrame(), selected_asset or "asset")),
            "intraday": _figure_json(build_intraday_comparison_chart(pd.DataFrame(), selected_asset or "asset", now)),
        },
        "asset_session_rows": [],
        "mapped_post_rows": [],
        "event_study_rows": [],
        ...
        "intraday_window_rows": [],
        "empty_states": {
            "asset_market": message,
            "mapped_posts": "",
            "event_study": "",
            "intraday": message,
        },
...
def build_research_asset_lab(
    *,
    ...
    mode = _comparison_mode(comparison_mode)
    ...
    resolved_pre_sessions = _bounded_int(pre_sessions, 3, minimum=1, maximum=10)
    resolved_post_sessions = _bounded_int(post_sessions, 5, minimum=1, maximum=10)
    resolved_before_minutes = _bounded_int(before_minutes, DEFAULT_INTRADAY_BEFORE_MINUTES, minimum=30, maximum=390)
    resolved_after_minutes = _bounded_int(after_minutes, DEFAULT_INTRADAY_AFTER_MINUTES, minimum=30, maximum=780)
    ...
    selected_features = _filter_asset_features(
    ...
    intraday_chart = build_intraday_comparison_chart(pd.DataFrame(), resolved_asset, pd.Timestamp.now(tz=settings.timezone))
    ...
    empty_states = {
        "asset_market": "" if not comparison.empty else f"No overlapping daily market data was found for SPY and {resolved_asset}.",
        "mapped_posts": "" if not asset_mapping_table.empty else f"No mapped posts for {resolved_asset} were found in the current filters.",
        "event_study": "" if not event_study.empty else f"No asset-specific event-study rows are available for {resolved_asset}.",
        "intraday": intraday_message,
    }
    ...
    return {
        ...
        "charts": {
            "overlay": _figure_json(build_asset_comparison_chart(comparison, resolved_asset, mode)),
            "event_study": _figure_json(build_event_study_chart(event_study, resolved_asset)),
            "intraday": _figure_json(intraday_chart),
        },
        "asset_session_rows": _records(asset_session_table.sort_values("trade_date", ascending=False) if not asset_session_table.empty else asset_session_table),
        "mapped_post_rows": _records(asset_mapping_table),
        "event_study_rows": _records(event_study),
        "intraday_anchor_options": anchor_options[:TABLE_LIMIT],
        "intraday_coverage_rows": _records(coverage, limit=None),
        "intraday_window_rows": _records(intraday_comparison),
